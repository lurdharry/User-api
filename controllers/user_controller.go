...
func CreateUser(c echo.Context) error  {
  ...
  if err != nil {
	return c.JSON(http.StatusInternalServerError,responses.UserResponse{
		Status: http.StatusInternalServerError,Message: "error", Data: &echo.Map{"data": err.Error()},
	})
  }

  return c.JSON(http.StatusCreated,responses.UserResponse{
	Status: http.StatusCreated,Message: "success", Data: &echo.Map{"data": result},
  })
}
...
func EditUser(c echo.Context) error {
	...
	result,err := userCollection.UpdateOne(ctx,bson.M{"d":objId},bson.M{"set":update})
	...
	return c.JSON(http.StatusOK,responses.UserResponse{
		Status: http.StatusOK,Message: "success",Data: &echo.Map{"data":updatedUser},
	})
}
...
func DeleteUser(c echo.Context) error  {
	...
	if result.DeletedCount < 1{
		return  c.JSON(http.StatusInternalServerError,responses.UserResponse{
			Status: http.StatusInternalServerError,
			Message: "error",
			Data: &echo.Map{"data":"User with specified id n"},
		})
	}
